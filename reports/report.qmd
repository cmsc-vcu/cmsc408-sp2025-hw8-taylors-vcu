...
FROM 
    (SELECT DISTINCT `Region` AS region FROM sp25_taylors_user.wdi_country) AS regions
CROSS JOIN 
    (SELECT DISTINCT `Income Group` AS income_group FROM sp25_taylors_user.wdi_country) AS income_groups
WHERE 
    NOT EXISTS (
        SELECT 1
        FROM sp25_taylors_user.wdi_country AS original
        WHERE original.`Region` = regions.region
          AND original.`Income Group` = income_groups.income_group
    )
ORDER BY 
    regions.region, income_groups.income_group;
""")
